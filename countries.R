...
REswa<-rep %>% 
  ...
  dplyr::filter (Country.Region=="Eswatini") %>% pull(R)

REswa<-c(REswa,rep(1,days-length(REswa)-1))

RNam<-rep %>% 
  dplyr::filter(days_infectious==7) %>% 
  mutate(Date=lubridate::ymd(Date))%>%
  dplyr::filter (Date>="2021-11-1") %>%
  dplyr::filter (Country.Region=="Namibia") %>% pull(R)

RNam<-c(RNam,rep(1,days-length(RNam)-1))
...
RSA<-rep %>% 
  ...
  mutate(Date=lubridate::ymd(Date))%>%
  dplyr::filter (Date>="2021-11-1") %>%
  dplyr::filter (Country.Region=="South Africa") %>% pull(R)

RSA<-c(RSA,rep(1,days-length(RSA)-1))
...
RLes <-rep %>% 
  ...
  mutate(Date=lubridate::ymd(Date))%>%
  dplyr::filter (Date>="2021-11-1") %>%
  dplyr::filter (Country.Region=="Lesotho") %>% pull(R)
