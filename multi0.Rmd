...
Based on the current information `r country` has not started their vaccination process. In this case, we work with scenarios where the vaccination covers 80% of the population during 180 days. This means we set range of maximum vaccines per day `r avg_vac_people` people vaccinated per day, which represents `r avg_vac daily` doses.
...
ta<-out_format %>% filter(final_coverage>min_coverage)%>% 
  ...
  mutate(deaths_averted=mean(deaths_averted,na.rm = T)) %>% 
    group_by(final_coverage,R0,max_vaccine) %>%
...
te<-out_format %>% filter(final_coverage>min_coverage)%>% 
  mutate(final_coverage = round(final_coverage,2))%>%
  select(final_coverage,R0,vaccine_coverage_mat,max_vaccine,deaths_averted) %>%
  group_by(R0,max_vaccine,vaccine_coverage_mat,final_coverage) %>%
  mutate(deaths_averted=mean(deaths_averted,na.rm = T)) %>% 
